...
async def callback(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    data = await controller.register_card()
    if data != "":
        job_removed = remove_job_if_exists(str(chat_id), context)
        text = (
            "Card registered"
            if job_removed
            else "You have no active card registration."
        )

        # add to db
        db_controller.add_keycard(chat_id, data)

        await context.bot.send_message(chat_id, text=f"{text}")
...
async def start_get_keycard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Place keycard on the card reader")
...
async def get_keycard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        context.job_queue.run_repeating(
            callback, interval=1, chat_id=chat_id, name=str(chat_id)
        )
