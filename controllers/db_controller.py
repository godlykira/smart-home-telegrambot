...
def remove_appliance(chat_id, appliance_index):
    """
    Removes an appliance from the user's data based on the provided chat_id and appliance_index.

    Parameters:
    - chat_id (int): The id of the chat/user.
    - appliance_index (int): The index of the appliance to be removed.
...
def update_appliance_status(chat_id, appliance_index):
    """
    Update the status of a specific appliance in the user data.

    Args:
        chat_id: The ID of the chat/user.
        appliance_index: The index of the appliance to update.
    ...
    with open(file_path, "r") as f:
        ...
        for item in db_data:
            if item["chat_id"] == chat_id:
                item["appliances"][int(appliance_index) - 1]["status"] = (
                    True
                    if item["appliances"][int(appliance_index) - 1]["status"] is False
                    else False
                )
...
def add_keycard(chat_id, keycard):
    """
    Updates the user's keycard in the database.

    Args:
        chat_id: The chat ID of the user.
        keycard: The keycard to be added or updated.
